...
function Blog({ isLoggedIn }) {
    ...
    useEffect(() => {
        const fetchFilteredResults = async () => {
            try {
                const response = await fetch('https://guzo-jet.vercel.app/search/blogs', {
                    method: 'GET',
                    headers: {
    ...
    return (
        <div className='flex flex-col w-full min-h-screen bg-white'>
            ...
            {
                blogResponse ? (
                    <div className="w-full flex flex-wrap space-x-4 items-center px-20 py-10 mt-20">
                        <p className='w-full text-center font-serif text-3xl text-black p-6'>Blogs</p>
                        {blogResponse.map((blog, index) => (
                            <Card key={index} shadow={false} className="relative grid h-[30rem] w-1/2 max-w-[24rem] items-end justify-center overflow-hidden text-center hover:scale-105">
                                <CardHeader floated={false} shadow={false} color="transparent" className="absolute inset-0 m-0 h-full w-full rounded-none bg-background2 bg-cover bg-center">
